import { Injectable, NotFoundException, HttpException, HttpStatus } from '@nestjs/common';
import { TipoProcesoRepository } from '../tipo-proceso.repository';

export interface EliminarTipoProcesoResponse {
  mensaje: string;
}

@Injectable()
export class EliminarTipoProcesoUseCase {
  constructor(private readonly repository: TipoProcesoRepository) {}

  async execute(id: string): Promise<EliminarTipoProcesoResponse> {
    try {
      const existente = await this.repository.obtenerPorId(id);
      if (!existente) throw new NotFoundException(`Tipo de proceso con ID ${id} no encontrado`);

      await this.repository.eliminar(id);
      return { mensaje: `Tipo de proceso ${existente.nombre} eliminado correctamente` };
    } catch (error) {
      if (error instanceof NotFoundException) throw error;
      throw new HttpException(
        { description: 'Error al eliminar el tipo de proceso', errorMessage: error.message },
        error.status || HttpStatus.INTERNAL_SERVER_ERROR,
      );
    }
  }
}
